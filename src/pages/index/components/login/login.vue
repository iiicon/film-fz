    ...
    <div class="login-box">
      <div class="login-content">
        ...
        <div class="bind-field">
          <div class="bind" @click="_dologin">登录</div>
        </div>
      </div>
    </div>
  </div>
...
import { getLoginCode, dologin } from 'api/login'
...
export default {
  ...
  methods: {
    ...
    getVerifyCode() {
      ...
      this.timer(60)
      getLoginCode(this.phone).then((data) => {
        if (data.success) {
          showToast(data.msg)
        }
      }).catch((e) => {
    ...
    _dologin() {
      ...
      dologin(this.phone, this.verifyCode).then((data) => {
        console.log(data)
        if (data.success) {
          showToast(data.msg)
          this.$router.back()
